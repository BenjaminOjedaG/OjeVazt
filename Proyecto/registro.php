...
$clavemaestra = "ojevazt2024"; //asignamos clave maestra del software
                        ...
                        if ($_POST["clavamaestra"] == $clavemaestra) { //comparamos clave maestra para lograr el registro
                            ...
                            if (mysqli_num_rows($chek) == 0) { //si no encuentra ningun usuario con ese nombre
                                ...
                                if (!file_exists('fotoperfil/' . $_FILES['fotoperfil']['name'])) { //si no existe ninguna foto con ese nombre se guardara la foto en la carpeta foto de perfil
                                    ...
                                    mysqli_query($bd, 'INSERT INTO Usuario (Usuario, Contraseña, Nombre, Correo, Foto_Perfil, Fecha_Nacimiento) VALUES ("' . $_POST["usuario"] . '","' . $_POST["pass"] . '","' . $_POST["nombre"] . '","' . $_POST["correo"]  . '","' . $_FILES['fotoperfil']['name'] . '","' . $_POST["fecha"] . '");');
                                    ...
                                    mysqli_query($bd, 'INSERT INTO usuario (Usuario, Contraseña, Nombre, Correo, Foto_Perfil, Fecha_Nacimiento) VALUES ("' . $_POST["usuario"] . '","' . $_POST["pass"] . '","' . $_POST["nombre"] . '","' . $_POST["correo"]  . '","' . $_POST["usuario"] . $_FILES['fotoperfil']['name'] . '","' . $_POST["fecha"] . '");');
                                    header("Location:index.php?causa=reg");
                                }
                            } else {
...
<head>
    ...
    <script src="/Ojevazt/Proyecto/js/script.js">
